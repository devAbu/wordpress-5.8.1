<?php 
	...
	$class = 'masonry-post '.esc_attr(funio_get_class()->class_item_blog);
?>
		...
		<?php if( empty($format) || $format == 'image' || $format == 'quote') : ?>	
			...
			<div class="entry-thumb single-thumb">
				<a class="post-thumbnail" href="<?php echo esc_url(the_permalink()); ?>" title="<?php the_title_attribute(); ?>">
					<?php the_post_thumbnail( 'funio-blog-layout-1' )?>				
				</a>
				<?php funio_posted_on(); ?>
			</div>
			<?php } ?>
		<?php elseif( $format == 'video' || $format == 'audio' ) : ?>
			<div class="entry-thumb single-thumb">
				<a class="post-thumbnail" href="<?php esc_url(the_permalink()); ?>" title="<?php the_title_attribute(); ?>">
					<?php the_post_thumbnail( 'funio-blog-layout-1', array( 'alt' => get_the_title() ) );	?>      
				</a>
				<?php funio_posted_on(); ?>
			</div>	
						...
						<div class="slick-carousel" data-columns4="1" data-columns3="1" data-columns2="1" data-columns1="1" data-columns="1" data-nav="true">
								<?php
									if($ids){
										...
										foreach ( $ids as $i => $id ){ ?>
											<div class="item">
													<?php echo wp_get_attachment_image($id, 'funio-blog-layout-1'); ?>
											</div>
										<?php }	
									}
								?>
			...
			<div class="entry-meta-head">
				<?php if (funio_get_config('archives-author')) { ?>
					<div class="entry-author">
						<span class="entry-meta-link"><?php echo esc_html__("By : ",'funio') ?><?php echo the_author_posts_link(); ?></span>
					</div>
				<?php } ?>
